class Vec3 {
  ...
  rotate() {
    let {x, y, z} = this.clone().sub(origin);
...
class Shape {
  constructor(points, {strokeColor="white", fillColor=color(1, .5), strokeWeight=1, showPoints = true,}) {
    ...
    for (let p of this.points) this.center.add(p);
  ...
  computeNormal() {
    if (this.points.length > 2) {
      let A = this.points[1].clone().sub(this.points[0]);
      let B = this.points[2].clone().sub(this.points[0]);
...
let Box = (anchor, w, l, h, style={}) => {
  let verts = [
    anchor,
    anchor.clone().add(new Vec3(w, 0, 0))
  ];
  verts = verts.concat(verts.map(p => p.clone().add(new Vec3(0, l, 0))));
  verts = verts.concat(verts.map(p => p.clone().add(new Vec3(0, 0, h))));
  let faces = [
    [0, 1, 3, 2], // bottom
    [4, 5, 7, 6], // top
    [0, 1, 5, 4], // +x
